...
export function handleStatusUpdated(event: StatusUpdated): void {
  ...
  // Handle the status.
  if (event.params.status == MINIPOOLSTATUS_STAKING) handleMinipoolStakingStatus(node, minipool, event.block.timestamp);
  else if (event.params.status == MINIPOOLSTATUS_WITHDRAWABLE) handleMinipoolWithdrawableStatus(node, minipool, event.block.timestamp);
...
export function handleEtherDeposited(event: EtherDeposited): void {
  // Preliminary null checks.
  if (event === null || event.params === null || event.address === null || event.block === null) return;
  ...
  if (
    ROCKET_NODE_DEPOSIT_CONTRACT_ADDRESS_V1 == event.params.from.toHexString() ||
 ...
 */
export function handleMinipoolStakingStatus(node: Node, minipool: Minipool, blockTimeStamp: BigInt): void {
 ...
 */
export function handleMinipoolWithdrawableStatus(node: Node, minipool: Minipool, blockTimeStamp: BigInt): void {
